<template>
  ...
  >
    <div class="relative size-full overflow-hidden">
      ...
      <div
        class="relative size-full bg-red-50 transition-transform duration-300 dark:bg-black"
        :class="imageContainerClass"
      >
        <img :src="imageUrl" alt="image" :class="imageClass" />
      </div>
      <transition name="fade">
        <div v-show="direction !== null" :class="childrenClass"></div>
      </transition>
    </div>
  </div>
</template>
...
const containerClass = computed(() =>
  cn(
    "group/card relative overflow-hidden rounded-3xl bg-transparent h-full w-full",
    props.class
  )
);
...
const imageClass = computed(() =>
  cn(
    "h-full w-full scale-125 object-cover transition-transform duration-300",
    props.imageClass
  )
);

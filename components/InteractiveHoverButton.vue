<template>
  <button
    ...
    :class="
      cn(
        'group relative w-auto cursor-pointer overflow-hidden rounded-full border border-base-content bg-background p-2 px-6 text-center font-semibold',
        props.class
      )
    "
  >
    <div class="flex items-center gap-2">
      <div
        class="size-2 scale-100 rounded-lg bg-black transition-all duration-300 group-hover:scale-[100.8]"
      ></div>
      <span
    ...
    >
      <span class="whitespace-nowrap text-base-content">{{ text }}</span>

      <Icon
        v-if="displayIcon"
        name="material-symbols:shopping-cart-checkout-rounded"
        class="w-6 h-6 bg-base-content"
      />
    </div>
  </button>
</template>
